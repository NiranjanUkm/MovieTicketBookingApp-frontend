...
const ProfilePage: FC = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            ...
            try {
                const response = await axios.get('http://localhost:4001/users/profile/getProfile', {
    ...
    const handleSubmit = async () => {
        ...
        try { 
            setLoading(true);
            await axios.put('http://localhost:4001/users/profile/updateProfile', form.values, {
                headers: { Authorization: `Bearer ${token}` } // Include Authorization header
            });
            setLoading(false);
        } catch (error) {
